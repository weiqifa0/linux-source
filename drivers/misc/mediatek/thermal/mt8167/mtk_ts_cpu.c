...
int tscpu_ts_temp_r[THERMAL_SENSOR_NUM];
...
static void thermal_reset_and_initial(void)
{
	...
	THERMAL_WRAP_WR32(0x00000D79, TEMPAHBPOLL);	/* poll is set to 31.25ms */
	...
	THERMAL_WRAP_WR32(0x000002FE, TEMPAHBPOLL);	/* poll is set to 6.94ms */
	...
	THERMAL_WRAP_WR32(0x00000563, TEMPAHBPOLL);	/* poll is set to 12.5ms */
	...
	THERMAL_WRAP_WR32(0x01C001C0, TEMPMONCTL2);	/* both filt and sen interval is 6.94ms */
	THERMAL_WRAP_WR32(0x000002FE, TEMPAHBPOLL);	/* poll is set to 458379*15.15= 6.94ms */
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	*t = (unsigned long)curr_temp;
	...
	if (isTimerCancelled) {
		...
		tscpu_printk("tscpu_get_temp isTimerCancelled=%d!\n", isTimerCancelled);
	...
	if (isTimerCancelled) {
		thermal->polling_delay = 0;
		tscpu_printk("tscpu_get_temp isTimerCancelled=%d!!\n", isTimerCancelled);
	} else if ((int)*t >= polling_trip_temp1)
		;
...
static enum hrtimer_restart tscpu_update_tempinfo(struct hrtimer *timer)
{
	...
	ktime = ktime_set(0, 100000000);	/* 100ms */
...
void tscpu_start_thermal_timer(void)
{
	...
	if (thz_dev != NULL && interval != 0) {
		thz_dev->polling_delay = interval;
		isTimerCancelled = 0;
	}
