...
#undef  KD_FUNC_ENTRY_EXIT_DEBUG
...
#define PK_XLOG_INFO(fmt, arg...)    pr_debug(fmt, ##arg)
...
#else
#define PK_DBG(a, ...)
#define PK_INFO(fmt, arg...)
#define PK_ERR(fmt, arg...)          pr_err(PFX "[%s] " fmt, __func__, ##arg)
#define PK_XLOG_INFO(fmt, arg...)
...
#define GPIO_CMPCLK_M_GPIO      (GPIO_MODE_00)
	...
	pr_debug("[CAMERA SENSOR] kdSetSensorGpio enable=%d, type=%d\n",
...
static int CAMERA_HW_probe(struct platform_device *pdev)
{
	...
	PK_ERR("cam1_rst:%d, cam1_pdn:%d\n", GPIO_CAM1_RST, GPIO_CAM1_PDN);
