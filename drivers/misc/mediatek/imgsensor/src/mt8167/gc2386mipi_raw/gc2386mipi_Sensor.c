...
#include "gc2386mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC2386_camera_sensor"
#define LOG_1 LOG_INF("GC2386,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

//#define LOG_INF(format, args...)    xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)    pr_err("[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2386DuringTestPattern = KAL_FALSE;

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2386MIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
    ...
    .mipi_settle_delay_mode = 1,//MIPI_SETTLEDELAY_MANNUAL,//0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL
...
{
	kal_uint32 temp = 0;
	temp = ((read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1));
	printk("temp = %x\n",temp);
    return (temp + 1);
}
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC2386MIPI analogic gain 1x, GC2386MIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 1.43x , GC2386MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 2x , GC2386MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 2.84x , GC2386MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 3.97x , GC2386MIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 5.68x , GC2386MIPI add pregain = %d\n",temp);
	}	
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 8.14x , GC2386MIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_8<= iReg)&&(iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 11.34x , GC2386MIPI add pregain = %d\n",temp);
	}	
	...
	else 
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2386MIPI analogic gain 16.23x , GC2386MIPI add pregain = %d\n",temp);
	}
	return gain;
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("2386 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
                printk(KERN_ERR "gc2386 open func : i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            printk(KERN_ERR "gc2386 open func : Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC2386DuringTestPattern = KAL_FALSE; 
...
UINT32 GC2386MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC2386MIPI_RAW_SensorInit    */
