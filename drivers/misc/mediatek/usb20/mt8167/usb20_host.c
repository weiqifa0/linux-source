...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on && !vbus_on) {
		...
		if (drvvbus_pin >= 0 && !IS_ERR(pinctrl) && !IS_ERR(pinctrl_drvvbus_high))
			pinctrl_select_state(pinctrl, pinctrl_drvvbus_high);
		else
			DBG(0, "Drive VBUS ON: drvvbus pin is not ready, ignore!\n");
#endif
#else
		...
		if (drvvbus_pin >= 0 && !IS_ERR(pinctrl) && !IS_ERR(pinctrl_drvvbus_low))
			pinctrl_select_state(pinctrl, pinctrl_drvvbus_low);
		else
			DBG(0, "Drive VBUS OFF: drvvbus pin is not ready, ignore!\n");
#endif
#else
		mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN, GPIO_OTG_DRVVBUS_PIN_M_GPIO);
		...
		DBG(0, "unexpect vbus status:is_on=%d, status:%d\n", is_on, vbus_on);
	}
#endif
}
...
void mt_usb_otg_init(struct musb *musb)
{
	...
	if (node == NULL) {
		...
		if (iddig_pin < 0) {
			...
			ret = of_property_read_u32(node, "iddig_gpio_mode", &iddig_pin_mode);
			if (ret < 0)
				DBG(0, "read iddig_pin_mode failed, can ignore...\n");
			else
		...
		if (drvvbus_pin < 0) {
			...
			ret = of_property_read_u32(node, "drvvbus_gpio_mode", &drvvbus_pin_mode);
			if (ret < 0)
				DBG(0, "read drvvbus_gpio mode failed, can ignore...\n");
			else
