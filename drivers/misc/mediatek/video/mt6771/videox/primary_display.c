...
#ifdef FREE_FB_BUFFER
static unsigned int init_dim_buffer(void)
{
	unsigned int mva = 0;
	...
	buffer_info = allocat_decouple_buffer(buffer_size);
	if (buffer_info != NULL) {
		mva = buffer_info->mva;
		memset(buffer_info->va, 0, buffer_size);
		DISPMSG("%s addr: 0x%x, size: 0x%x\n", __func__,
			mva, buffer_size);
		kfree(buffer_info);
	}
...
int primary_display_free_fb_buf(void)
{
	...
	if (pgc->framebuffer_pa) {
		...
		free_reserved_area((void *)va_start, (void *)va_end, 0xffff,
				   "fbmem");
