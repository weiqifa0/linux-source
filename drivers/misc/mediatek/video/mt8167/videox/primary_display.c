...
int primary_display_set_later_destroy_memory_session(unsigned int session_id)
{
	pgc->later_destroy_memory_session = 1;
	pgc->later_destroy_memory_session_id = session_id;
	return 0;
}
...
{
	struct disp_session_config memory_session_config;

	DISPDBG("primary_display_switch_mode sess_mode %d, session 0x%x\n", sess_mode, session);

	/* to destroy undestroyed memory session at suspend state */
	if (pgc->later_destroy_memory_session && !primary_display_is_sleepd()) {
		memset((void *)&memory_session_config, 0, sizeof(struct disp_session_config));
		memory_session_config.session_id = pgc->later_destroy_memory_session_id;
		if (disp_destroy_session(&memory_session_config) != 0) {
			DISP_LOG_E("later destroy memory session failed\n");
		} else if (disp_get_session_number() == 1) {
			primary_display_set_secondary_display(0, DISP_SESSION_MEMORY);
			pgc->later_destroy_memory_session = 0;
		}
	}
