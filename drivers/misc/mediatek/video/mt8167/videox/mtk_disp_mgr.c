...
int _ioctl_destroy_session(unsigned long arg)
{
	...
	if (config.type == DISP_SESSION_MEMORY) {
		...
		/* do nothing because primary suspend at dc state */
		/* the memory session will destroy after resume */
		if (primary_display_is_sleepd()) {
			DISPMSG("destroy memory session 0x%x after resume\n", config.session_id);
			primary_display_set_later_destroy_memory_session(config.session_id);
			return 0;
		}
	}
#endif
