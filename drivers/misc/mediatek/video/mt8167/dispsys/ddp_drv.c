...
static int disp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		...
		if (of_property_read_u32_index(np, "reg", 1, &reg_value))
			DDPERR("get reg value from DTS fail\n");
		if (of_property_read_u32_index(np, "interrupts", 1, &irq_value))
			DDPERR("get interrupts from DTS fail\n");
