...
int ddp_dsi_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			DISPDBG("[DSI]enter cmp DSI+0x78=0x%x\n",
				AS_UINT32(DDP_REG_BASE_DSI0 + 0x78));
			DISPDBG("[DSI]enter cmp DSI+0x7c=0x%x\n",
				AS_UINT32(DDP_REG_BASE_DSI0 + 0x7c));
			...
			packet_type = read_data0.byte0;
			DISPPRINT("DSI read packet_type is 0x%x\n", packet_type);

			if (packet_type == 0x1A || packet_type == 0x1C) {
				recv_data_cnt = read_data0.byte1 + read_data0.byte2 * 16;
				DISPCHECK("Set receive data count is %d bytes\n", recv_data_cnt);
				...
				for (j = 0; j < recv_data_cnt; j++) {
				   ...
				   packet_type == 0x12 || packet_type == 0x21 ||
				   packet_type == 0x22) {
				/* short read response */
				...
				if (recv_data_cnt > dsi_params->lcm_esd_check_table[i].count) {
					DISPCHECK
					("DSI read short packet data exceeds buffer size return size %d\n",
					dsi_params->lcm_esd_check_table[i].count);
					recv_data_cnt = dsi_params->lcm_esd_check_table[i].count;
				}
			...
			if (result == 0) {
				/* clear rx data */
				DISPPRINT("esd check pass!\n");
				ret = 0; /* esd pass */
			} else {
				DISPPRINT("esd check fail!\n");
				ret = 1; /* esd fail */
				break;
			}
		}
