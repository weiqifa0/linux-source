...
unsigned int ucDDRPHY_Register_Read(unsigned int u4reg_addr)
{
	unsigned int pu4reg_value;

	pu4reg_value = readl(IOMEM(DDRPHY_BASE_ADDR + u4reg_addr));
...
unsigned int get_dram_data_rate(void)
{
	...
	pu4reg_value = (ucDDRPHY_Register_Read(DDRPHY_SHU1_PLL5) >> 24) & 0x7f;

	return (pu4reg_value * 26);
}
EXPORT_SYMBOL(get_dram_data_rate);

void __iomem *mt_dramc_nao_chn_base_get(int channel)
{
	return DRAMCNAO_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_nao_chn_base_get);

void __iomem *mt_dramc_chn_base_get(int channel)
{
	return DRAMCAO_BASE_ADDR;
}
EXPORT_SYMBOL(mt_dramc_chn_base_get);

void __iomem *mt_ddrphy_chn_base_get(int channel)
{
