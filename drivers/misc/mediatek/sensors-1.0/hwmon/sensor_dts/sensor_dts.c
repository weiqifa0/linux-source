...
int get_hmdy_dts_func(const char *name, struct hmdy_hw *hw)
{
	...
	u32 power_vol[] = {0};
	u32 firlen[] = {0};
	u32 is_batch_supported_hmdy[] = {0};
	u32 is_batch_supported_temp[] = {0};
	...
	if (node) {
		...
		ret = of_property_read_u32_array(node, "is_batch_supported_hmdy", is_batch_supported_hmdy,
			ARRAY_SIZE(is_batch_supported_hmdy));
		if (ret == 0)
			hw->is_batch_supported_hmdy		 = is_batch_supported_hmdy[0];

		ret = of_property_read_u32_array(node, "is_batch_supported_temp", is_batch_supported_temp,
			ARRAY_SIZE(is_batch_supported_temp));
		if (ret == 0)
			hw->is_batch_supported_temp		 = is_batch_supported_temp[0];
	} else {
		SENSOR_PR_ERR("Device Tree: can not find gyro node!. Go to use old cust info\n");
		return -1;
	}
