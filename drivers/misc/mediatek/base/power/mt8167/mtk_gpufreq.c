...
{
	gpufreq_pr_debug("get_immediate_gpu_wrap doesn't exist");
	return 0;
}
	...
	if (gpu_speed_bounding == 0) {
		...
		if (!node) {
			...
			if (!of_property_read_u32(node, "clock-frequency", &gpu_speed))
				...
				gpufreq_pr_err("@%s: missing clock-frequency property, use default GPU level\n", __func__);
				gpu_speed = 400000;	/* default speed */
			}
		}
...
{
	while (1) {
		gpufreq_pr_debug("@%s: begin\n", __func__);

		if (g_pGpufreq_input_boost_notify != NULL) {
			gpufreq_pr_debug("@%s: g_pGpufreq_input_boost_notify\n", __func__);
			g_pGpufreq_input_boost_notify(g_gpufreq_max_id);
		}

		gpufreq_pr_debug("@%s: end\n", __func__);
 ...
 * Input boost
 **************************************************************************************/
static void mt_gpufreq_input_event(struct input_handle *handle, unsigned int type, unsigned int code, int value)
{
	...
	if ((type == EV_KEY) && (code == BTN_TOUCH) && (value == 1) && (mt_gpufreq_input_boost_state == 1)) {
		gpufreq_pr_debug("@%s: accept.\n", __func__);
		wake_up_process(mt_gpufreq_up_task);
	}
}

static int mt_gpufreq_input_connect(struct input_handler *handler, struct input_dev *dev, const struct input_device_id *id)
{
...
static void mt_gpufreq_power_calculation(unsigned int idx, unsigned int freq, unsigned int volt, unsigned int temp)
{
...
#define GPU_ACT_REF_VOLT		90000	/* mV x 100 */
	...
	p_dynamic = p_dynamic *
		((freq*100)/ref_freq) * ((volt*100)/ref_volt) * ((volt*100)/ref_volt) / (100*100*100);
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
	...
	if (mt_gpufreq_ptpod_disable == true) {
		if (enable == 0) {
			gpufreq_pr_debug("mt_gpufreq_ptpod_disable == true\n");
			return DRIVER_NOT_READY;
		}
	}
...
void mt_gpufreq_enable_by_ptpod(void)
{
	...
	if (g_pGpufreq_mfgclock_disable_notify)
		...
		gpufreq_pr_err("mt_gpufreq_enable_by_ptpod: no callback!\n");
...
void mt_gpufreq_disable_by_ptpod(void)
{
	...
	if (g_pGpufreq_mfgclock_enable_notify)
		...
		gpufreq_pr_err("mt_gpufreq_disable_by_ptpod: no callback!\n");
...
void mt_gpufreq_restore_default_volt(void)
{
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		...
		gpufreq_pr_debug("@%s: mt_gpufreqs[%d].gpufreq_volt = %x\n", __func__, i, mt_gpufreqs[i].gpufreq_volt);
	}
...
unsigned int mt_gpufreq_update_volt(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	if (array_size > mt_gpufreqs_num) {
		gpufreq_pr_err("mt_gpufreq_update_volt: array_size = %d, Over-Boundary!\n", array_size);
		return 0;
	}
...
unsigned int mt_gpufreq_get_volt_by_idx(unsigned int idx)
{
	...
	if (idx < mt_gpufreqs_num) {
		gpufreq_pr_debug("@%s: idx = %d, voltage= %d\n", __func__, idx, mt_gpufreqs[idx].gpufreq_volt);
...
static void mt_update_gpufreqs_power_table(void)
{
	...
	if ((temp >= -20) && (temp <= 125)) {
		for (i = 0; i < mt_gpufreqs_num; i++) {
			...
			gpufreq_pr_debug("update mt_gpufreqs_power[%d].gpufreq_khz = %d\n", i, mt_gpufreqs_power[i].gpufreq_khz);
			gpufreq_pr_debug("update mt_gpufreqs_power[%d].gpufreq_volt = %d\n", i, mt_gpufreqs_power[i].gpufreq_volt);
			gpufreq_pr_debug("update mt_gpufreqs_power[%d].gpufreq_power = %d\n", i, mt_gpufreqs_power[i].gpufreq_power);
		}
	} else
...
static void mt_setup_gpufreqs_power_table(int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		mt_gpufreq_power_calculation(i, mt_gpufreqs_power[i].gpufreq_khz, mt_gpufreqs_power[i].gpufreq_volt, temp);
		...
		gpufreq_pr_debug("mt_gpufreqs_power[%d].gpufreq_khz = %u\n", i, mt_gpufreqs_power[i].gpufreq_khz);
		gpufreq_pr_debug("mt_gpufreqs_power[%d].gpufreq_volt = %u\n", i, mt_gpufreqs_power[i].gpufreq_volt);
		gpufreq_pr_debug("mt_gpufreqs_power[%d].gpufreq_power = %u\n", i, mt_gpufreqs_power[i].gpufreq_power);
	}
...
static int mt_setup_gpufreqs_table(struct mt_gpufreq_table_info *freqs, int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		gpufreq_pr_debug("freqs[%d].gpufreq_khz = %u\n", i, freqs[i].gpufreq_khz);
		gpufreq_pr_debug("freqs[%d].gpufreq_volt = %u\n", i, freqs[i].gpufreq_volt);
		gpufreq_pr_debug("freqs[%d].gpufreq_idx = %u\n", i, freqs[i].gpufreq_idx);
	}
...
int mt_gpufreq_state_set(int enabled)
{
	if (enabled) {
		...
		gpufreq_pr_debug("enable GPU DVFS: g_gpufreq_dvfs_disable_count = %d\n", g_gpufreq_dvfs_disable_count);
		...
		/* Disable GPU DVFS */
		g_gpufreq_dvfs_disable_count++;
		gpufreq_pr_debug("disable GPU DVFS: g_gpufreq_dvfs_disable_count = %d\n", g_gpufreq_dvfs_disable_count);
...
/* #ifdef VGPU_SET_BY_PMIC */
	...
	if (volt_new > volt_old)
		regulator_set_voltage(mt_gpufreq_pmic->reg_vgpu, volt_new*10, (PMIC_MAX_VGPU*10) + 125);
	else
...
static void mt_gpufreq_set(unsigned int freq_old, unsigned int freq_new, unsigned int volt_old, unsigned int volt_new)
{
	gpufreq_pr_debug("mt_gpufreq_set:old volt=%d, new volt=%d, old freq=%d, new freq=%d", volt_old, volt_new, freq_old, freq_new);
...
unsigned int mt_gpufreq_target(unsigned int idx)
{
	...
	if (mt_gpufreq_volt_enable_state == 0) {
		gpufreq_pr_debug("mt_gpufreq_volt_enable_state == 0! return\n");
		mutex_unlock(&mt_gpufreq_lock);
		return DRIVER_NOT_READY;
	}
#ifdef MT_GPU_DVFS_RANDOM_TEST
	...
	if (mt_gpufreq_ptpod_disable == true) {
		...
		gpufreq_pr_debug("PTPOD disable dvfs, mt_gpufreq_ptpod_disable_idx = %d\n", mt_gpufreq_ptpod_disable_idx);
	}
...
void mt_gpufreq_oc_callback(enum BATTERY_OC_LEVEL oc_level)
{
	...
	if (g_limited_oc_ignore_state == true) {
		gpufreq_pr_debug("@%s: g_limited_oc_ignore_state == true!\n", __func__);
		return;
	}
...
void mt_gpufreq_low_batt_volume_callback(enum BATTERY_PERCENT_LEVEL low_battery_volume)
{
	...
	if (g_limited_low_batt_volume_ignore_state == true) {
		gpufreq_pr_debug("@%s: g_limited_low_batt_volume_ignore_state == true!\n", __func__);
		return;
	}
...
void mt_gpufreq_low_batt_volt_callback(enum LOW_BATTERY_LEVEL low_battery_level)
{
	...
	if (g_limited_low_batt_volt_ignore_state == true) {
		gpufreq_pr_debug("@%s: g_limited_low_batt_volt_ignore_state == true!\n", __func__);
		return;
	}
...
void mt_gpufreq_thermal_protect(unsigned int limited_power)
{
	...
	if (g_limited_thermal_ignore_state == true) {
		gpufreq_pr_debug("@%s: g_limited_thermal_ignore_state == true!\n", __func__);
		mutex_unlock(&mt_gpufreq_power_lock);
		return;
	}
	...
	if (mt_gpufreq_prev_thermal_limited_freq != limited_freq) {
		...
		if (limited_freq < GPU_DVFS_FREQ5)
			gpufreq_pr_debug("@%s: p %u f %u i %u\n", __func__, limited_power, limited_freq, mt_gpufreq_power_limited_index_array[IDX_THERMAL_LIMITED]);
	}
...
void mt_gpufreq_set_power_limit_by_pbm(unsigned int limited_power)
{
	...
	if (g_limited_pbm_ignore_state == true) {
		gpufreq_pr_debug("@%s: g_limited_pbm_ignore_state == true!\n", __func__);
		mutex_unlock(&mt_gpufreq_power_lock);
		return;
	}
...
static void _mt_gpufreq_fixed_volt(int fixed_volt)
{
	/* volt (mV) */
#ifdef VGPU_SET_BY_PMIC
	if (fixed_volt >= (PMIC_MIN_VGPU / 100) && fixed_volt <= (PMIC_MAX_VGPU / 100)) {
#endif
...
static ssize_t mt_gpufreq_lpt_enable_proc_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
