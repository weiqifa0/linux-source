...
void spm_sodi_set_sramrom_on(bool on)
{
	if (on)
		__spm_sodi.pwrctrl->param1 |= PWR_CTRL_PARAM1_SRAMROM;
	else
		__spm_sodi.pwrctrl->param1 &= ~PWR_CTRL_PARAM1_SRAMROM;
}

static bool spm_sodi_is_sramrom_on(void)
{
	if (__spm_sodi.pwrctrl->param1 & PWR_CTRL_PARAM1_SRAMROM)
		return true;
	else
		return false;
}
...
void spm_go_to_sodi(u32 spm_flags, u32 spm_data)
{
	...
	spm_flags |= SPM_NO_PMIC_WRAP;
#endif

	if (spm_sodi_is_sramrom_on())
		spm_flags |= SPM_SRAMROM_PDN_DIS;
