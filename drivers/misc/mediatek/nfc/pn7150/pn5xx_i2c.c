		...
		size_t count, loff_t *offset)
{
	...
	//printk("%s : colin  writing %zu bytes.  ", __func__, count);
	while(jj < count){
	//printk(",0x%x",tmp[jj]);	
	jj++;
	}
	...
	if (ret != count) {
		...
		pr_err("%s : colin i2c_master_send colin returned %d  %x\n", __func__, ret,pn54x_dev->client->addr);
		if(count == 6 && tmp[0] == 3 && tmp[1] == 0 && tmp[2] == 3 && tmp[3] == 0x81 && tmp[4] == 2 && tmp[5] == 4)
			ret = count;
		else
		ret = -EIO;
			
	}else{
		//printk("%s : colin  writing %zu bytes. ok! ", __func__, count);
		//while(1){
		//ret = i2c_master_send(pn54x_dev->client, tmp, count);
        //tmp[ret] = '\0';		
		//mdelay(2000);
		//pr_err("%s : colin  i2c_master_send colin returned %d     %s\n", __func__, ret,tmp);
		//}
	}
