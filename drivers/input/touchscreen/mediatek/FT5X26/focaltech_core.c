...
static ssize_t read_proc(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	len = snprintf(buf, 10, "%s", mProcData);
	return len;
}
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	case PROC_UPGRADE: {
		...
		snprintf(upgrade_file_path, WRITE_BUF_SIZE, "%s", writebuf + 1);
...
static int tpd_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strncpy(info->type, TPD_DEVICE, 20);
...
static int ctp_proc_read_show(struct seq_file *m, void *data)
{
	...
	*/
	...
	snprintf(temp, 40, "[Vendor]%s,[Fw]%x,[IC]FT5446\n", "O-Film", 4);
	seq_printf(m, "%s\n", temp);
	return 0;
}
