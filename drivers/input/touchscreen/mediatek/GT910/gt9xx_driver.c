...
static unsigned int fw_version = -1;
static unsigned int config_version =-1;
static struct proc_dir_entry *gt91xx_fw_version = NULL;
static struct proc_dir_entry *gt91xx_config_version = NULL;
...
#define GT91XX_PROC_FW_VERSION     "gt9xx_fw_version"
#define GT91XX_PROC_CONFIG_VERSION     "gt9xx_config_version"
...
#undef CONFIG_GTP_USE_PINCTRL  /*workaround modify by medaitek, you should disable it in xxx_defconfig&debug_defconfig file*/
#endif
...
{
   return tpd_load_status;
}

static int of_get_gt9xx_platform_data(struct device *dev)
{
	
	GTP_ERROR("of_get_gt9xx_platform_data \n");
...
void gtp_gpio_output(int gpio_type, int level)
{
	...
	if (gpio_type == GTP_RST_GPIO) {
		tpd_gpio_output(GTP_RST_PORT, level);/*add by mediatek*/
		gpio_direction_output(tpd_rst_gpio, level);
	} else if (gpio_type == GTP_IRQ_GPIO) {
		tpd_gpio_output(GTP_INT_PORT, level);/*add by mediatek*/
...
void gtp_gpio_input(int gpio_type)
{
	...
	if (gpio_type == GTP_RST_GPIO) {
		...
		tpd_gpio_as_int(GTP_INT_PORT);/*add by mediatek*/
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	GTP_INFO("add Sensor_ID: %d", sensor_id);
...
#else
	{
		...
		config_version = send_cfg_buf[sensor_id][0];
...
static ssize_t gt91xx_fw_version_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos){
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
	//char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = { 0 };
...
static ssize_t gt91xx_config_version_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos){
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
...
static const struct file_operations gt_fw_version_fops = {
	.read = gt91xx_fw_version_read_proc
};

static const struct file_operations gt_config_version_fops = {
	.read = gt91xx_config_version_read_proc
};
...
#if 1
		irq_enabled = true; //解决立马进入touch_event_handler然后enable_irq导致不平衡报warning by yangning
#endif
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
	GTP_ERROR("tpd_i2c_probe to get gpio resources");
	...
	gt91xx_fw_version = proc_create(GT91XX_PROC_FW_VERSION, 0777, NULL,&gt_fw_version_fops);
	if (gt91xx_fw_version == NULL) 
		GTP_ERROR("create_proc_entry %s failed\n",GT91XX_PROC_FW_VERSION);
	
	gt91xx_config_version = proc_create(GT91XX_PROC_CONFIG_VERSION, 0777, NULL,&gt_config_version_fops);
	if (gt91xx_config_version == NULL)
		GTP_ERROR("create_proc_entry %s failed\n",GT91XX_PROC_CONFIG_VERSION);

#ifdef CONFIG_GTP_CREATE_WR_NODE
	init_wr_node(client);
#endif
//161012
#if 0 //polling mode for test
...
//				}
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				input_x = TPD_WARP_X(abs_y_max, input_x);
				input_y = TPD_WARP_Y(abs_x_max, input_y);
#else
