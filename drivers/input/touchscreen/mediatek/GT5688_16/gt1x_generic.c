...
#endif
#define CONFIG_GTP_DRIVER_SEND_CFG
/*******************GLOBAL VARIABLE*********************/
...
#ifdef FTS_TP_NAME //linhui.yanrenjie modified to compatible GTXX 20190517
static unsigned short device_firmware_version =01;
static unsigned short device_firmware_id = 0x00;
...
#ifdef FTS_TP_NAME
void set_device_firmware_version(unsigned short data)
{
	device_firmware_version = data;
}

void set_device_firmware_id(unsigned short data)
{
	device_firmware_id = data;
}
...
static ssize_t tp_name_proc_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
    ...
    ptr += sprintf(ptr, "TP:GTXXXX,hwid:%04x,FW:%04x\n", device_firmware_id, device_firmware_version);
...
#if LAVA_GESTURE_NODE_EN 
#define CTP_GESTURE_MENU_PROC_FILE      "hotknot_menu" 
static struct proc_dir_entry *ctp_gesture_menu_proc = NULL;
#endif

#if LAVA_GESTURE_NODE_EN
static ssize_t gt91xx_gesture_menu_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    unsigned long ctp_gesture_flag = 0; 
	s32 ret = 0;
    u8 ges_data[10] = {};
    ...
    if (copy_from_user(&ges_data[0], buffer, count))
    {
        return -EFAULT;
    }  
    ret = kstrtoul(&ges_data[0], 10, &ctp_gesture_flag);
	if (ret) 
    {
            return -EINVAL;
	}	
    if (ctp_gesture_flag)
        gt1x_gesture_mode_set(ENABLE);
    else 
        gt1x_gesture_mode_set(DISABLE);
    return count;
}


static ssize_t gt91xx_gesture_menu_read_proc(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int err = -1;
...
static const struct file_operations gt_gesture_menu_proc_fops = { 
    .write = gt91xx_gesture_menu_write_proc,
    .read = gt91xx_gesture_menu_read_proc,
};

unsigned long get_gt1x_ctp_gesture_flag(void)
{
    return gt1x_gesture_mode_get();
}
...
s32 gt1x_send_cfg(u8 * config, int cfg_len)
{
    ...
    if (update_info.status) {
        GTP_ERROR("Ignore cfg during fw update.");
        return -1;
    }
    
    mutex_lock(&mutex_cfg);
	GTP_DEBUG("Driver send config, length:%d,config:0x%x 0x%x", cfg_len,config[0],config[1]);
    ...
    if (config[0x805A - GTP_REG_CONFIG_DATA] & 0x40) {
        ...
        GTP_ERROR("ext_cfg, debug info:%X", config[GTP_CONFIG_ORG_LENGTH]);
		...
		if (ret < 0) {
	 	...
	 	return -1;
         	...
         	GTP_ERROR("Send ext_config successfully");
 		}
    }
 ...
 */
 //extern char *mtkfb_find_lcm_driver_new(void);
s32 gt1x_init_panel(void)
{
	...
	//const u8 cfg_grp0_new[] = GTP_CFG_GROUP0_NEW;
	const u8 *cfgs[] = {
	...
	#if 0
	if(strcmp("aeon_rm67200_fhd_dsi_vdo_w519_lide", mtkfb_find_lcm_driver_new()) == 0){
	   printk("aeon_rm67200_fhd_dsi_vdo_w519_lide-cfg_grp0\n"); 
	   cfgs[0] = cfg_grp0;
	}
	else if(strcmp("aeon_hx8399_fhd_dsi_vdo_w519_sharp", mtkfb_find_lcm_driver_new()) == 0){
	   printk("aeon_hx8399_fhd_dsi_vdo_w519_sharp-cfg_grp0_new\n"); 
	   cfgs[0] = cfg_grp0_new;
	}	
	#endif
    
	printk("Config groups length:%d,%d,%d,%d,%d,%d\n", cfg_lens[0], cfg_lens[1], cfg_lens[2], cfg_lens[3], cfg_lens[4], cfg_lens[5]);
	...
	printk("Config group%d used, length:%d\n", sensor_id, cfg_len);
...
s32 gt1x_init(void)
{
	...
	while (retry++ < 5) {
		...
		#endif
#if LAVA_GESTURE_NODE_EN 
			ctp_gesture_menu_proc = proc_create(CTP_GESTURE_MENU_PROC_FILE, 0666, NULL, &gt_gesture_menu_proc_fops);
			
			if (ctp_gesture_menu_proc == NULL)
			{
				GTP_ERROR("create_proc_entry %s failed\n", CTP_GESTURE_MENU_PROC_FILE);
			}
#endif
	...
	if (gt1x_chip_type == CHIP_TYPE_GT1X)
	{	
		set_device_firmware_id(0x01);//GT1X
		//set_device_firmware_version(cfg_len);
	}
	else if (gt1x_chip_type == CHIP_TYPE_GT2X)
	{
		set_device_firmware_id(0x02);//GT2X
		//set_device_firmware_version(cfg_len);
	}
	else
	{
		set_device_firmware_id(0xFF);//NOT SUPPORT
		//set_device_firmware_version(cfg_len);
	}
#endif	
	if (ret != 0) {
