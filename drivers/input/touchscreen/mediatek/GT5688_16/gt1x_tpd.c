...
void gt1x_irq_disable(void)
{
    ...
    if (tpd_irq_flag) {
	 ...
	 disable_irq_wake(tpd_touch_irq);
#else
...
ssize_t get_cfg_version(void)
{
	s32 ret = 0;
	char buf[2] = {0};
	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, buf, sizeof(buf));
	tpd_firmware_version[0] = (int)(buf[0]);
	return 0;
}
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_capability(tpd->dev, EV_KEY, TPD_GES_KEY_DOUBLECLICK); 
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_C);   //ugrec_tky
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_U);//lava as double click key
	...
	//tpd_load_status = 1;//yanrenjie modified for compatible other TP driver.
	get_cfg_version();
	return 0;
}
...
static int tpd_local_init(void)
{
	...
	if (tpd_load_status == 0)	// disable auto load touch driver for linux3.0 porting
	{
		...
		free_irq(tpd_touch_irq, NULL);//INONE yanrenjie added to fix the bug when you use the other TP the system will reboot forever 20190430
