...
void set_bat_sw_cv_charging_current_limit(int current_limit)
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] set_bat_sw_cv_charging_current_limit (%d)\r\n",
		    current_limit);
#if defined(CONFIG_MTK_FAN5405_SUPPORT)
        ulc_cv_charging_current = charging_set_sw_cv_current(current_limit);
#else
...
static void pchr_sw_cv_charing_current_check(void)
{
	...
	if (csdac_full_flag == KAL_TRUE && ulc_cv_charging_current != CHARGE_CURRENT_0_00_MA) {
...
PMU_STATUS BAT_TopOffModeAction(void)
{
	...
	if (BMT_status.bat_vol > (cv_voltage + 10)  && 0/*(cv_voltage - CV_CHECK_DELAT_FOR_BANDGAP)*/) {	/* CV - 0.08V */
