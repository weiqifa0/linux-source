	...
	struct snd_kcontrol *kcontrol, int event)
{
    ...
    snd_soc_update_bits(codec, AUDIO_CODEC_CON03, BIT(15), 0x1);
	snd_soc_update_bits(codec, AUDIO_CODEC_CON03, BIT(16), 0x1);
	temp = snd_soc_read(codec,
				AUDIO_CODEC_CON03);
	temp = temp | 0x00018000;
	printk("colin:%x\n",temp);
    snd_soc_write(codec, AUDIO_CODEC_CON03, temp);
	return 0;
}
