...
static void tas5782m_power(struct device *dev, struct tas5782m_priv *priv, bool enable)
{
...
static int tas5782m_probe(struct device *dev, struct regmap *regmap)
{
	...
	if (!dev->of_node) {
		pr_warn("%s, cannot find dts node!\n", __func__);
		return ret;
	}
	...
	if (!gpio_is_valid(tas5782m->gpio_power))
		pr_warn("%s get invalid tas5782m_power_gpio %d\n",
	...
	if (!gpio_is_valid(tas5782m->gpio_reset))
		pr_warn("%s get invalid tas5782m_rst_gpio %d\n",
	...
	if (ret != 0) {
		pr_warn("Failed to initialize TAS5782M: %d\n", ret);
		goto err;
	}
	...
	ret = snd_soc_register_codec(dev,
								...
								1);
	if (ret != 0) {
		pr_warn("Failed to register CODEC: %d\n", ret);
		goto err;
	}
...
int tas5782m_speaker_amp_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
