...
static const unsigned int src_param[Soc_Aud_I2S_SAMPLERATE_I2S_NUM][AUDIO_SRC_PARAM_TYPE_NUM] = {
	...
	[Soc_Aud_I2S_SAMPLERATE_I2S_130K] = {SRC_PARAM_INVALID, SRC_PARAM_INVALID, SRC_PARAM_INVALID},
	...
	[Soc_Aud_I2S_SAMPLERATE_I2S_260K] = {SRC_PARAM_INVALID, SRC_PARAM_INVALID, SRC_PARAM_INVALID},
};
...
bool set_general_asrc_1_parameter(unsigned int sample_rate_in, unsigned int sample_rate_out)
{
	...
	unsigned int src_in_freq_mode = src_param[src_in_mode][AUDIO_SRC_PARAM_TYPE_FREQ_MODE];
	unsigned int src_out_freq_mode = src_param[src_out_mode][AUDIO_SRC_PARAM_TYPE_FREQ_MODE];
	...
	if (src_out_freq_mode != SRC_PARAM_INVALID) {
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON3,
				src_param[src_out_mode][AUDIO_SRC_PARAM_TYPE_FREQ_MODE], 0x00ffffff);
	}
	if (src_in_freq_mode != SRC_PARAM_INVALID) {
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON4,
				src_param[src_in_mode][AUDIO_SRC_PARAM_TYPE_FREQ_MODE], 0x00ffffff);
	}
	...
	if (sample_rate_in > sample_rate_out) {
		const unsigned int *iir_coeff = get_iir_coeff(sample_rate_in, sample_rate_out, &iir_coeff_num);
		...
		/* Clear coeff history to read/write coeff from the first position */
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON13, 0x0 << 0, 0x3f << 0);
		/* Write SRC coeff */
		for (coeff_index = 0; coeff_index < iir_coeff_num; coeff_index++) {
			/* Can not use Afe_Set_Reg because it leads another read and moves the coeff position next */
			Afe_Set_Reg_Val(AFE_GENERAL1_ASRC_2CH_CON12, iir_coeff[coeff_index]);
		}
#ifdef DEBUG_COEFF
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON13, 0x0 << 0, 0x3f << 0);
		...
		for (coeff_index = 0; coeff_index < iir_coeff_num; coeff_index++) {
			pr_debug("%s(), coeff_index = %d, AFE_GENERAL1_ASRC_2CH_CON12 = 0x%x\n",
					__func__, coeff_index, Afe_Get_Reg(AFE_GENERAL1_ASRC_2CH_CON12));
		}
#endif
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON0, 0x0 << 1, 0x1 << 1);
		/* CHSET_IIR_STAGE */
		Afe_Set_Reg(AFE_GENERAL1_ASRC_2CH_CON2, ((iir_coeff_num / 6) - 1) << 8, 0x7 << 8);
		/* CHSET_IIR_EN */
...
bool set_general_asrc_2_parameter(unsigned int sample_rate_in, unsigned int sample_rate_out)
{
	unsigned int src_in_mode = SampleRateTransform(sample_rate_in, 0);
	unsigned int src_out_mode = SampleRateTransform(sample_rate_out, 0);
	...
	if (sample_rate_in > sample_rate_out) {
		...
		for (coeff_index = 0; coeff_index < iir_coeff_num; coeff_index++) {
			/* Can not use Afe_Set_Reg because it leads another read and moves the coeff position next */
			Afe_Set_Reg_Val(AFE_GENERAL2_ASRC_2CH_CON12, iir_coeff[coeff_index]);
		}
#ifdef DEBUG_COEFF
		Afe_Set_Reg(AFE_GENERAL2_ASRC_2CH_CON13, 0x0 << 0, 0x3f << 0);
		...
		for (coeff_index = 0; coeff_index < iir_coeff_num; coeff_index++) {
			pr_debug("%s(), coeff_index = %d, AFE_GENERAL2_ASRC_2CH_CON12 = 0x%x\n",
					__func__, coeff_index, Afe_Get_Reg(AFE_GENERAL2_ASRC_2CH_CON12));
		}
#endif
		Afe_Set_Reg(AFE_GENERAL2_ASRC_2CH_CON0, 0x0 << 1, 0x1 << 1);
		/* CHSET_IIR_STAGE */
		Afe_Set_Reg(AFE_GENERAL2_ASRC_2CH_CON2, ((iir_coeff_num / 6) - 1) << 8, 0x7 << 8);
		/* CHSET_IIR_EN */
		...
		unsigned int sample_rate_in, unsigned int sample_rate_out)
{
	...
	switch (id) {
	case AUDIO_GENERAL_ASRC_1:
		ret = set_general_asrc_1_parameter(sample_rate_in, sample_rate_out);
		break;
	case AUDIO_GENERAL_ASRC_2:
		ret = set_general_asrc_2_parameter(sample_rate_in, sample_rate_out);
		break;
	default:
		pr_debug("%s(), No such SRC id = %d\n", __func__, id);
	}
