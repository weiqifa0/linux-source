...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_EXTADC_ON] = {"aud_extadc_on", false, NULL},
	[GPIO_EXTADC_OFF] = {"aud_extadc_off", false, NULL},
};
#endif
...
int AudDrv_GPIO_EXTADC_Select(int mode)
{
	int retval = 0;

	switch (mode) {
	case 0:
		if (AudDrv_GPIO_IsValid(GPIO_EXTADC_OFF))
			retval = AudDrv_GPIO_Select(GPIO_EXTADC_OFF);
		break;
	case 1:
		if (AudDrv_GPIO_IsValid(GPIO_EXTADC_ON))
			retval = AudDrv_GPIO_Select(GPIO_EXTADC_ON);
		break;
	default:
		pr_debug("%s(), invalid mode = %d", __func__, mode);
		retval = -1;
