...
#ifdef CONFIG_SND_SOC_USING_SRAMROM
#include "mtk_spm_idle.h"
#endif

#define DEBUG (1)

#define PCM_TAG "[PCM]: %s() line:%d "
#define PCM_DEBUG(fmt,args...) printk(KERN_INFO PCM_TAG fmt,__FUNCTION__,__LINE__,##args)
...
#ifdef CONFIG_SND_SOC_ES7243X
extern void es7243_off_or_standby(int enable);
extern int es7243_tdm_en ;
#endif
	...
	int bit_width)
{
	...
	PCM_DEBUG("fs:%d rate:%d bit_width:%d\n",fs,rate,bit_width);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	PCM_DEBUG("rate:%d apply_i2s_out_change:%d apply_i2s_in_change:%d\n",rate,apply_i2s_out_change,apply_i2s_in_change);
			       ...
			       struct snd_soc_dai *dai)
{
	...
	dev_dbg(afe->dev, "=== weiqifa === dai_name:%s\n",dai->name);
				...
				struct snd_soc_dai *dai)
{
	...
	PCM_DEBUG("phy_width:%d channels:%d",phy_width,channels);
	if (be->prepared[stream]) {
		dev_info(afe->dev, "%s prepared already\n", __func__);
		return 0;
	}
				...
				struct snd_soc_dai *dai)
{
	...
	dev_err(afe->dev, "%s %s\n", __func__, memif->data->name);
	PCM_DEBUG("es7243 rate:%d channels:%d sample_bits:%d\n",dai->rate,dai->channels,dai->sample_bits);
#ifdef CONFIG_SND_SOC_ES7243X
	if(strcmp("TDM_IN", memif->data->name) == 0){
		es7243_tdm_en = 1;
		es7243_off_or_standby(1);
		PCM_DEBUG("set es7243.\n");
	}
	PCM_DEBUG("set es7243.end\n");
#endif
				  ...
				  struct snd_soc_dai *dai)
{
	...
	if (request_size > data->max_sram_size) {
		dev_info(afe->dev, "%s now Using DRAM\n", __func__);
...
#ifdef CONFIG_SND_SOC_USING_SRAMROM
		if (afe->use_sramrom && data->max_sram_size == 55 * 1024) {
			dma_buf->area = ((unsigned char *)afe->sram_address1) +
				   data->sram_offset;
			dma_buf->addr = afe->sram_phy_address1 + data->sram_offset;
			spm_sodi_set_sramrom_on(1);
			spm_dpidle_set_sramrom_on(1);
			dev_info(afe->dev, "%s now %d using SRAMROM 55k\n", __func__, data->id);
		} else {
#endif
			dma_buf->area = ((unsigned char *)afe->sram_address) +
				   data->sram_offset;
			dma_buf->addr = afe->sram_phy_address + data->sram_offset;
			dev_info(afe->dev, "%s now %d using SRAM 36k\n", __func__, data->id);
#ifdef CONFIG_SND_SOC_USING_SRAMROM
		}
		spm_dpidle_set_pll_ldo_on(1);
#endif
...
#ifdef CONFIG_SND_SOC_USING_SRAMROM
	if (memif->data->max_sram_size == 55 * 1024) {
		spm_sodi_set_sramrom_on(0);
		spm_dpidle_set_sramrom_on(0);
	}
#endif
...
static const struct snd_kcontrol_new mt8167_afe_o11_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I05 Switch", AFE_CONN3, 7, 1, 0),
};
...
static int mt8167_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	if (!IS_ERR(afe->sram_address)) {
		...
		dev_info(&pdev->dev, "SRAM initialized, addr(0x%x), size(0x%x)\n",
			afe->sram_phy_address, afe->sram_size);
	}

#ifdef CONFIG_SND_SOC_USING_SRAMROM
	res = platform_get_resource(pdev, IORESOURCE_MEM, 2);
	afe->sram_address1 = devm_ioremap_resource(&pdev->dev, res);
	if (!IS_ERR(afe->sram_address1)) {
		afe->sram_phy_address1 = res->start;
		afe->sram_size1 = resource_size(res);
		afe->use_sramrom = true;
		dev_info(&pdev->dev, "SRAMROM initialized, addr(0x%x), size(0x%x)\n",
			afe->sram_phy_address1, afe->sram_size1);
	}
#endif
