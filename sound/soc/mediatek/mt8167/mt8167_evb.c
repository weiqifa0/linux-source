...
enum PINCTRL_PIN_STATE {
	...
	PIN_STATE_AMP_PWR_ON,
	PIN_STATE_AMP_PWR_OFF,
	PIN_STATE_MAX
};
...
static void mt8167_evb_ext_spk_amp_turn_on(struct snd_soc_card *card)
{
	...
	if (!IS_ERR(card_data->pin_states[PIN_STATE_AMP_PWR_ON])){
		ret = pinctrl_select_state(card_data->pinctrl,
		card_data->pin_states[PIN_STATE_AMP_PWR_ON]);
		if (ret)
		dev_err(card->dev, "%s PIN_STATE_AMP_PWR_ON failed to select state %d\n",
			__func__, ret);
	}
...
static void mt8167_evb_ext_spk_amp_turn_off(struct snd_soc_card *card)
{
	...
	printk("=== weiqifa === %s-%d\n",__FUNCTION__,__LINE__);
	if (!IS_ERR(card_data->pin_states[PIN_STATE_AMP_PWR_OFF])){
				ret = pinctrl_select_state(card_data->pinctrl,
			card_data->pin_states[PIN_STATE_AMP_PWR_OFF]);
			if (ret)
			dev_err(card->dev, "%s PIN_STATE_AMP_PWR_OFF failed to select state %d\n",
				__func__, ret);
		}
...
static const struct snd_soc_dapm_widget mt8167_evb_dapm_widgets[] = {
	...
	SND_SOC_DAPM_INPUT("External I2S in"),	
};
...
static const struct snd_soc_dapm_route mt8167_evb_audio_map[] = {
	...
	{"AIF RX", NULL, "AFE_CLK"},
	{"AIF RX", NULL, "DA_CLK"},
	{"External I2S out", NULL, "2ND I2S Playback"},
	{"2ND I2S Capture", NULL, "External I2S in"},
};
...
static struct snd_soc_dai_link mt8167_evb_dais[] = {
	...
	},
	{
		...
		.cpu_dai_name = "TDM_IN",
		#if 0//def CONFIG_SND_SOC_ES7243
		.codec_name = "es7243.2-0013",
		.codec_dai_name = "ES7243 HiFi 0",
		#else
		.codec_name = "snd-soc-dummy",
		.codec_dai_name = "snd-soc-dummy-dai",		
